...
const onSubmitBook = async e => {
  ...
  await axios.post('https://salty-shelf-14169.herokuapp.com/api/books', {
    title: title.value,
    author: author.value,
...
const getBooks = async () => {
  const res = await axios.get('https://salty-shelf-14169.herokuapp.com/api/books')
  const data = res.data
  setBooks(data)
}
...
const onSubmitEdits = async (e, id) => {
  e.preventDefault()
  const {title, author, description} = e.target
  await axios.post(`https://salty-shelf-14169.herokuapp.com/api/books/update/${id}`, {
    title: title.value,
    author: author.value,
    description: description.value,
  })
...
const deleteBook = async bookToDelete => {
  await axios({
    method: 'DELETE',
    url: 'https://salty-shelf-14169.herokuapp.com/api/books/',
    data: {
      id: bookToDelete,
    },
  })
